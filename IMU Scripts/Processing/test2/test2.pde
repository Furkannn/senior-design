...
void calculateDisplacement() {
  readIMUData();
  roll_disp = n_roll - roll;
  yaw_disp = -1 * (n_yaw - yaw);
  pitch_disp = n_pitch - pitch; 
}

void drawLine() {
  calculateDisplacement();
  //line(screen_width/2, screen_height/2, screen_width/2 + 10.0*(yaw_disp), screen_height/2 + 10.0*(pitch_disp));
  line(0, 0, 10.0*(yaw_disp), 10.0*(pitch_disp));
  stroke(255);
  strokeWeight(10);
}
...
void keyPressed() {
  switch (key) {
    ...
    println("n_roll: " + n_roll + " n_pitch: " + n_pitch);
